...
const cookieParser = require('cookie-parser');
 ...
 async function registerController(req, res) {
    ...
    // const isUserExistsByEmail = await userModel.find({email});
    
    // if(isUserExistsByEmail) {
    //     return res.status(409).json({
    //         message: 'User already exista with same email'
    //     });
    // }

    // const isUserExistsByUsername = await userModel.find({username})
    ...
    if(isUserAlreadyExists) {
        return res.status(409).json({
            message: 'User already exists'+ (isUserAlreadyExists.email ===
                 email ? ' with same email' : ' with same username')
        });
    }
